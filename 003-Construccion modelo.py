...
# Predicción en TRAIN
prob_train = round(pd.DataFrame(modelo.predict_proba(X = X_train))[1],2)
gini(y_train,prob_train)

pred_train = modelo.predict(X = X_train)
metricas_eficiencia(y_train,pred_train)


# Predicción en TEST
prob_test = round(pd.DataFrame(modelo.predict_proba(X = X_test))[1],2)
gini(y_test,prob_test)

pred_test = modelo.predict(X = X_test)
metricas_eficiencia(y_test,pred_test)


var_resp=y_test.reset_index().copy().fuga

tabla_eficincia(prob_test,var_resp,10)